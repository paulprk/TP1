...
$authMiddleware = function (Request $request, $handler) {
    ...
    // 2. Obtener Authorization header correctamente
    ...
    try {
        ...
        // 9. Continuar con la request original
        return $handler->handle($request);

    } catch (PDOException $e) {

        // 10. Error del servidor
        $response = new \Slim\Psr7\Response();
        $response->getBody()->write(json_encode([
            "error" => "Error en el servidor"
        ]));
...
// Verifica:
// 1. Que exista el header Authorization (Bearer TOKEN)
// 2. Que el token sea válido
// 3. Que el token no haya expirado
// 4. Renueva la sesión (vuelve a 5 minutos)

// Ejemplo de uso en una ruta protegida:

// $app->get('/ruta-protegida', function (Request $request, Response $response) {
//     $response->getBody()->write(json_encode([
